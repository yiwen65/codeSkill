...
- Accept optional argument: `init`, `resume`, `safe`, or `dev`.
...
```bash
python3 .agents/skills/spec-dev-loop/scripts/plan_to_backlog.py docs/PLAN.yaml .agent_state/backlog.json
```
...
3. Run task tests from `test_plan` (or `scripts/run_tests.sh` fallback).
4. Retry fix+retest up to 3 cycles when failing.
5. Mark `done` or `blocked` with reason.
